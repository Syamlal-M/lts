...
const LeaveForecast = () => {
  ...
  const [
    processedLeaveForcastData,
    setProcessedLeaveForcastData
  ] = React.useState([]);
  ...
  const columns = [
    {
      field: 'id', headerName: '#', filterable: false,
      renderCell: (index) => index.api.getRowIndex(index.row.empId) + 1,
    },
    { field: 'empId', headerName: 'Emp ID' },
    { field: 'employeeName', flex: 0.2, headerName: 'Name' },
    { field: 'org', flex: 0.2, headerName: 'Org' },
    { field: 'team', flex: 0.2, headerName: 'Team' },
    {
      field: 'datePicker', minWidth: 200, flex: 0.3, headerName: 'Select Dates', renderCell: ({ row }) =>
        <DatePicker
          multiple
          format="YYYY-MM-DD"
  ...
  return (
    <div className="home-main-div">
      ...
      />
      <TextField
        style={{ left: "10%", top: "50px", marginLeft: "20px" }}
        id="team"
        label="Team"
        variant="outlined"
      ...
      <Button
        id="search"
        variant="contained"
        style={{ left: "15%", top: "60px", marginLeft: "20px" }}
      ...
      {uploadSuccess &&
        <Alert severity="success" style={{ marginTop: "5%", marginLeft: "50%" }} className='alerts'>Leaves updated successfully</Alert>
      }

      {uploadError &&
        <Alert severity="error" className='alerts' style={{ marginTop: "5%", marginLeft: "50%" }}>Failed to update the leaves</Alert>
      }
