...
const SelectListProvider = ({ children }: SelectListProviderProps) => {
  ...
  const processSelectList = useCallback(
    (dataList: string[]): KeyValueObject[] => {
      return [
        DEFAULT_SELECT_ITEM,
        ...dataList.map((listItem) => ({ label: listItem, value: listItem }))
      ];
    },
    [DEFAULT_SELECT_ITEM]
  );
  ...
  return <SelectListContext.Provider value={value}>{children}</SelectListContext.Provider>;
};
